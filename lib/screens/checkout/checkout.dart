...
class CheckoutScreen extends StatefulWidget {
  ...
  CheckoutScreen({
    ...
    required this.chargeBay, required this.location, required this.documentId, required this.paymentMethodName,
  });
...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  /// Fetch the correct Firestore document
  ...
  double calculateTotalAmount() {
    ...
    if (widget.chargeBay == "Hour") {
      List<String> amountParts = widget.amount.split(" / ");
      double initialRate = double.tryParse(amountParts[0]) ?? 0.0;
      double subsequentRate = double.tryParse(amountParts[1]) ?? 0.0;
  ...
  Future<void> updateBookingDetails(BuildContext context) async {
    ...
    try {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => QRScreen(bookingDetail: bookingDetail,documentId: widget.documentId,),
        ),
      );
